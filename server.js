...
const prompts = require("./constant/prompt2");
...
dispatcher.onPost("/make-call", async function (req, res) {
  ...
  const client = twilio(process.env.TWILIO_ACCOUNT_SID, process.env.TWILIO_AUTH_TOKEN);
    ...
    const call = await client.calls.create({
        ...
        to: correctNumber,
        method: 'POST',
        url: `https://01a7-171-78-215-102.ngrok-free.app/twiml`,
      });
...
dispatcher.onPost("/twiml", function (req, res) {
  ...
  console.log({req : req?.params?.From});
  res.writeHead(200, {
...
async function promptLLM(mediaStream, prompt) {
  ...
  const filePath = path.join(__dirname, 'messages.json');
  const data = fs.readFileSync(filePath, 'utf8');
  const stage = await checkPrompt(prompt);
  console.log({stage});

  const chatLen = history.length;
  let currentPrompt = prompts?.introduction;
  if(stage == 'introduction'){
     currentPrompt = prompts?.introduction;
  }
  if(stage == 'referral'){
     currentPrompt = prompts?.referral;
     //currentPrompt = currentPrompt.replace("[*ClientAddress*]", address);
  }
  if(stage == 'confirming_ownership_details'){
     currentPrompt = prompts?.confirming_ownership_details;
     //currentPrompt = currentPrompt.replace("[*ClientAddress*]", address);
  }
  if(stage == 'property_details_collection'){
     currentPrompt = prompts?.propertyDetailsCollection;
     //currentPrompt = currentPrompt.replace("[*ClientAddress*]", address);
  }
  if(stage == 'property_details_verification'){
     currentPrompt = prompts?.property_details_verification;
     //currentPrompt = currentPrompt.replace("[*ClientAddress*]", address);
  }
  if(stage == 'fee_structure_explanation'){
     currentPrompt = prompts?.feeStructureExplanation;
     //currentPrompt = currentPrompt.replace("[*ClientAddress*]", address);
  }
  if(stage == 'process_explanation_inspection_details_and_pricing'){
     currentPrompt = prompts?.process_explanation_inspection_details_and_pricing;
     //currentPrompt = currentPrompt.replace("[*ClientAddress*]", address);
  }
  if(stage == 'renovation_inquiry'){
     currentPrompt = prompts?.renovation_inquiry;
     //currentPrompt = currentPrompt.replace("[*ClientAddress*]", address);
  }
  if(stage == 'personalization_and_reference'){
     currentPrompt = prompts?.personalizationAndReference;
     //currentPrompt = currentPrompt.replace("[*ClientAddress*]", address);
  }
  if(stage == 'report_delivery_promise'){
     currentPrompt = prompts?.reportDeliveryPromise;
     //currentPrompt = currentPrompt.replace("[*ClientAddress*]", address);
  }
  if(stage == 'closing'){
     currentPrompt = prompts?.closing;
     //currentPrompt = currentPrompt.replace("[*ClientAddress*]", address);
  }
  if(chatLen == 0){
     currentPrompt = prompts?.introduction;
  ...
  const instruction = [{
    role: "system",
    content : currentPrompt // jsonData?.prompt
  }]
...
const checkPrompt = async (input) =>{
  try {
     ...
     const instruction = [{
        role: "system",
        content : prompts?.checkPrompt
      }];
      history?.map(async (chat)=>{
         
        if(chat?.sender == 'user'){
          instruction.push({role: "user",content : chat?.msg});
          
        }   
        if(chat?.sender == 'bot'){
          instruction.push({role: "assistant",content : chat?.msg});
     ...
     const response = await openai.chat.completions.create({
      ...
      if(isJsonString(result)){
        ...
        return data?.next_stage;
        
      }else{
        const correctJson = extractJSON(result);
        const data = JSON.parse(correctJson);
        return data?.next_stage;
      }
